...
type TOCLinkPropT = { children: React.ReactNode; isActive: boolean; href?: string; onClick?: () => void }

const TOCLink = React.forwardRef<HTMLAnchorElement, TOCLinkPropT>(function link({ children, isActive, href, onClick, ...props }, ref) {
  return (
    <a href={href} onClick={onClick} style={{ fontWeight: isActive ? 'bold' : 'normal' }} ref={ref} {...props}>
      {children}
    </a>
  )
})
    ...
    <MainLayout>
      <Container maxW="8xl">
        <Flex width="full" gap={'10px'}>
          ...
          <Flex width={'20%'} direction="column" borderLeft="1px" borderColor="gray.200">
            <Box pos="sticky" top="10" px="2">
              <Text> On this Page</Text>
              <VStack align="flex-start">
                {nestedHeadings.map(({ id, title, items }, i) => (
                  <Link key={i} href={`#${id}`} scroll={false} passHref legacyBehavior>
